...
int main(int argc, char *argv[])
{
    ...
    if(argc < 3){//Error handler for not passing any argument (file)
        write(1 , "error no argument include !" , sizeof("error no argument include !"));
        return 1;
    }
    ...
    if(fd < 0){ //Error handler for the first argument (file)
        write(1 , "error opening file 1 !" , sizeof("error opening file 1 !"));
        return 1;
    }

    int fdc = open(argv[2],O_RDWR);

    if(fdc < 0){ //Error handler for the second argument (file)
        write(1 , "error opening file 2 !" , sizeof("error opening file 2 !"));
        return 1;
    }
